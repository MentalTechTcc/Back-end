...
import { EntityArrayResponseType, EspecialidadeService } from '../service/especialidade.service';
...
export class EspecialidadeComponent implements OnInit {
  ...
  trackId = (_index: number, item: IEspecialidade): number => this.especialidadeService.getEspecialidadeIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.especialidades = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IEspecialidade[] | null): IEspecialidade[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.especialidadeService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
